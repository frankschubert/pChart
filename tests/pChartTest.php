...
class pChartTest extends PHPUnit_Framework_TestCase {
    ...
    public function testFlatPieGraph() {
        ...
        $Test->drawFlatPieGraphWithShadow($DataSet->GetData(), $DataSet->GetDataDescription(), 120, 100, 60, PIE_PERCENTAGE, 8, 0, $shadowProperties);
        $Test->drawPieLegend(230, 15, $DataSet->GetData(), $DataSet->GetDataDescription(), new Color(250));

        file_put_contents(dirname(__FILE__) . '/action_logs/testFlatPieGraph', $canvas->getActionLog());

        $this->assertEquals('6fb645a6318014dc9978a1e77ccb9079', md5($canvas->getActionLog()));
    }
    ...
    public function testDrawBasicPieGraph() {
        ...
        $this->assertEquals(array(0 => array('Serie1' => 10,
            ...
            4 => array('Serie1' => 3,
                'Name' => 4,
                'Serie2' => 'May')), $DataSet->getData());

        $this->assertEquals(array(0 => 'Serie1',
            1 => 'Serie2'), $DataSet->getDataDescription()->values);
        ...
        // Draw the pie chart
        $Test->setFontProperties("Fonts/tahoma.ttf", 8);
        $Test->drawBasicPieGraph($DataSet->GetData(), $DataSet->GetDataDescription(), 120, 100, 70, PIE_PERCENTAGE, new Color(255, 255, 218), 0, ShadowProperties::NoShadow());
        $Test->drawPieLegend(230, 15, $DataSet->GetData(), $DataSet->GetDataDescription(), new Color(250));
    ...
    public function testDrawFilledLineGraph() {
        // Dataset definition 
        $DataSet = new pData;
        CSVImporter::importFromCSV($DataSet, dirname(__FILE__)
    ...
    public function testDrawOverlayBarGraph() {
        ...
        $Test->drawLegend(600, 30, $DataSet->GetDataDescription(), new Color(255));
        $Test->setFontProperties("Fonts/tahoma.ttf", 10);
        $Test->drawTitle(50, 22, "Example 3", new Color(50), 585);

        $this->assertEquals('8c6b50852e15c261e6a40595eed7e283', md5($canvas->getActionLog()));
    }
